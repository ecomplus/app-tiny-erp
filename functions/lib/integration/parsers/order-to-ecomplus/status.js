module.exports = (situacao, mapStatus, storeId) => {
  ...
  if (Array.isArray(mapStatus) && mapStatus.length) {
    ...
    const currentStatus = mapStatus.find(({tiny_status}) => (tiny_status && tiny_status.toLowerCase()) === (situacao && situacao.toLowerCase()))
    if (4566 == storeId) {
